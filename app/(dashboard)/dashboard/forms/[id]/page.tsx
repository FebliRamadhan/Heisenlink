...
import { ArrowLeft, ExternalLink, BarChart2, Copy } from "lucide-react"
...
export default function FormEditorPage({ params }: { params: { id: string } }) {
    ...
    const { data: form, isLoading, isError } = useQuery<Form>({
        queryKey: ["form", id],
        queryFn: async () => {
    ...
    const setTab = (next: string) => router.replace(`/dashboard/forms/${id}?tab=${next}`)
    ...
    if (isError || !form) {
        return (
            <div className="flex-1 p-8 pt-6">
                ...
                <Link href="/dashboard/forms" className="text-primary hover:underline">Back to forms</Link>
            </div>
        )
    }

    const togglePublish = (checked: boolean) => updateMutation.mutate({ isPublished: checked })
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <Input
                        ...
                        onBlur={() => title !== form.title && updateMutation.mutate({ title })}
                ...
                <div className="flex items-center gap-3">
                    <label className="flex items-center gap-2 text-sm">
                        <Switch checked={form.isPublished} onCheckedChange={togglePublish} />
                        {form.isPublished ? "Published" : "Draft"}
                    </label>
                    <Button variant="outline" size="sm" onClick={() => { navigator.clipboard.writeText(form.url); toast({ description: "Link copied" }) }}>
                    ...
                    <Link href={`/dashboard/forms/${id}/responses`}>
                        <Button variant="outline" size="sm"><BarChart2 className="mr-2 h-4 w-4" /> Responses ({form.responseCount})</Button>
                    </Link>
                </div>
            </div>
